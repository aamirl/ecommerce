...
Images.prototype = {
	get single() {
		...
		return {
	    	...
	    	// 			return _s_config.paths.images.products.main + (obj.type ? 'medium' : obj.type) + '/' + keys[ keys.length * Math.random() << 0];
	    				return _s_config.paths.images.products.main + (obj.type ? 'medium' : obj.type) + '/' + (data[0].image?data[0].image:data[0]);
	    				}
	    			else{

	    				if(obj.dets){
		    				return {
		    					small : _s_config.paths.images.products.main + 'small/' + data,
								medium : _s_config.paths.images.products.main + 'medium/' + data,
								large : _s_config.paths.images.products.main + 'large/' + data,
								details : obj.dets
		    					}
	    					}
	    				else {
